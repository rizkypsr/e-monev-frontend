...
const TriwulanCreate = () => {
  ...
  const procurementTypeQuery = useInfiniteQuery({
    queryKey: ['get_procurement_type'],
    queryFn: ({ pageParam = 1 }) =>
      getProcurementType(initialParams, authHeader()),
    getNextPageParam: (lastPage) => lastPage.nextId ?? undefined,
    getPreviousPageParam: (firstPage) => firstPage.previousId ?? undefined,
  });

  const procurementMethodQuery = useInfiniteQuery({
    queryKey: ['get_procurement_method'],
    queryFn: ({ pageParam = 1 }) =>
      getProcurementMethod(initialParams, authHeader()),
    getNextPageParam: (lastPage) => lastPage.nextId ?? undefined,
    getPreviousPageParam: (firstPage) => firstPage.previousId ?? undefined,
  });
